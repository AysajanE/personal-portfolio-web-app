...
Where:
  ...
  - $p_{jt}$ is the **price** for room type $j$ in booking period $t$.
  - $D_{jt}(p_{jt})$ is the expected **demand** (or number of bookings) at that price.
  - $C_{j}$ is the total **capacity** (number of available rooms) for room type $j$.
